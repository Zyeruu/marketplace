...
public final class Validator {
    ...
    private static final Pattern USER_NAME = Pattern.compile("^(?!.* {2})(?=.*\\s)[a-zA-Z ]{5,50}$");
    ...
    public static void isValidUserName(String userName) {
        if (!USER_NAME.matcher(userName).matches())
            throw new IllegalArgumentException("[!] The name provided has an invalid format.\n" +
                    "[!] Symbols are not supported.\n" +
                    "[!] The name must include a middle name or last name.");
    }

    public static void isValidStoreName(String storeName) {
        if (!STORE_NAME.matcher(storeName).matches())
            throw new IllegalArgumentException("[!] The store name provided has an invalid format.\n" +
                    "[!] Symbols and spaces are not supported.");
    }
}
